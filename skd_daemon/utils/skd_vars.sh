...
function SKD_SET_LOG_FILES(){
    ...
    # MAIN_LOG_DIR="${SKD_HOME_DIR}/evaluation/${EXP_NAME}/eval_${BENCH}/perflog-"$(date +"%Y%m%d-%H%M%S")
...
function SKD_BUILD_SKDCMD()
{
    ...
    SKD_CMD="perf stat -e task-clock,context-switches,cycles,instructions -x, -o ${SKDSTATFILE} ${SKD_HOME_DIR}/sk_daemon/tracker_d.o -p ${BENCHMARK_PID}"
...
function SKD_EXECUTE_TRACKER_D(){
    ...
    perf record -d ${SKD_PERF_EVENTS} -c ${SKD_FREQ} -p ${BENCHMARK_PID} -o - | perf script -F trace: -F time,addr --reltime -i - | ${SKD_CMD} 2>&1 | tee -a ${SKD_LOG}
