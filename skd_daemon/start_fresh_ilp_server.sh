...
if [ ${REMOTE_MODE} -eq 1 ]; then
    ...
    # tmux_command perf stat -x, -o ${ILP_PERF_STATS} -e ${ILP_PERF_EVENTS} --  ${ILP_HOME_DIR}/build/skd_ilp_server ${TCO_SAVE_PERCENT} |tee -a ${ILP_LOG}
    tmux_command "perf stat -x, -o ${ILP_PERF_STATS} -e ${ILP_PERF_EVENTS} -- bash -c '${ILP_HOME_DIR}/build/skd_ilp_server ${TCO_SAVE_PERCENT} 2>&1' | tee -a ${ILP_LOG}"
