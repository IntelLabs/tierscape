...
with open(args.input_file, "r") as f:
	for line in f:
		if f"{args.metric} " in line and not "not counted" in line:
			# replace multiple consecutive spaces with a single space
			line = ' '.join(line.split())
			cols = line.split(' ')
			metric_data.append(int(cols[1].replace(',', '')))
...
format_axis(ax, lfont-2)
plt.tight_layout()

if args.plot_pdf==1:
    # fit the plot to the figure
    print("Saving plot to: ", directory + "/plots/perftrend_" + args.metric + ".png")
    plt.savefig(directory + "/plots/perftrend_" + args.metric + ".png")
elif args.plot_pdf==2:
    print("Saving plot to: ", directory + "/plots/perftrend_" + args.metric + ".pdf")
    plt.savefig(directory + "/plots/perftrend_" + args.metric + ".pdf")
else:
