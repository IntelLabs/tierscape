...
extern char migrate_log_file[200];
...
TIERS_INFO::TIERS_INFO() {
    ...
    // int  tier_lat=0, phy_id=0, virt_id=0;
    int iscpu = 1;
    ...
    // Four NUMA nodes.
    // tiers.insert(tiers.begin(), new TierInfo(0, DRAM, "na", "na",dram_backing_store, iscpu, dram_tier_cost, 1));
    tiers.insert(tiers.begin(), new TierInfo(1, DRAM, 2));
    // tiers.insert(tiers.begin(), new TierInfo(2, OPTANE, "na", "na",optane_backing_store, iscpu, optane_tier_cost, 3));
    ...
    for (struct TierInfo* t : tiers) {
        ...
        fprintf(stderr, "Tier: %d type %s comp %s BS %d compression_ratio %f cost %d latency %d\n", t->get_virt_tier_id(), get_mem_type_string(t->type), t->compressor.c_str(), t->backing_store
            , t->compression_ratio, t->tier_cost, t->tier_latency);
    }
}
...
int push_a_region(REGION_SKD* curr_region, int pid, int dst_virt_tier) {
    ...
    */
    int ret = syscall(SYS_do_migrate_dst_tier, pid, aligned_start_address, page_len_bytes, dst_virt_tier-COMPRESSED_TIERS_BASED);
    if (ret) {
