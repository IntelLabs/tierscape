...
python3.8 ./plotting/plot_tiers_stats.py -i ${TIER_STATS_FILE} --pdf $SAVE_PDF
python3.8 ./plotting/plot_numastat.py -i "$MAIN_LOG_DIR/numastat" --pdf $SAVE_PDF -ts ${TREND_SLEEP_DURATION}
python3.8 ./plotting/plot_stacked_tco.py -d $MAIN_LOG_DIR"/plots" --pdf $SAVE_PDF -ts ${TREND_SLEEP_DURATION}

python3.8 ./plotting/plot_skdlog.py -i ${SKD_LOG} --pdf $SAVE_PDF
python3.8 ./plotting/plot_psi.py -i "$MAIN_LOG_DIR/psimemory" --pdf $SAVE_PDF -ts ${TREND_SLEEP_DURATION}
python3.8 plotting/plot_pcm_bw.py -i ${MAIN_LOG_DIR}/pcm_memory --pdf $SAVE_PDF -ts ${TREND_SLEEP_DURATION}
python3.8 plotting/plot_perftrend_metric.py -i ${MAIN_LOG_DIR}/perf_trend -p 1 -ts 1 -m mem_load_retired.local_pmm
python3.8 plotting/plot_vmstat.py -i ${MAIN_LOG_DIR}/vmstat -m pgmigrate_success -p 1
python3.8 plotting/plot_status.py -i ${MAIN_LOG_DIR}/status -m VmRSS -p 1

python3.8 ./plotting/plot_ops_and_avgops.py -i ${BENCH_EXEC_FILE} --pdf $SAVE_PDF

python3.8 ./plotting/plot_stacked_regions.py -i ${SKD_REGIONS} --pdf $SAVE_PDF
...
if [ -f ${PEBS_RAW_EVENTS}.gz ]; then
    ...
    python3.8 plotting/scatter_based_pebs_raw.py -i ${PEBS_RAW_EVENTS} --pdf $SAVE_PDF
...
# plotting/post_process.sh $PERF_FINAL_STATS $ILP_PERF_STATS 2>&1 | tee ${MAIN_LOG_DIR}/plots/post_process.log


sudo chown -R sandeep *
