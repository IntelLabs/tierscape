function GEN_RAW_EVENT(){
    ...
    perf script -F trace: -F time,addr --reltime -i ${PEBS_OUTFILE} -f > ${PEBS_RAW_EVENTS}
...
function START_PEBS_HOTNESS_LOGGING(){
    ...
    PERF_RECORD_CMD="perf record -d -c ${PEBS_C_FREQ} -e cpu/event=0xd0,umask=0x81/ppu -e cpu/event=0xd0,umask=0x82/ppu -e cpu/event=0xd0,umask=0x11/ppu -e cpu/event=0xd0,umask=0x12/ppu -p $BENCHMARK_PID  -o ${PEBS_OUTFILE}"
...
function START_PERF_END_TO_END() {
    ...
    echo "Starting perf stat (dat) " 
    CMD="perf stat -x, -o $PERF_FINAL_STATS -e $PERF_EVENTS -p $BENCHMARK_PID"
    # echo $CMD 
    $CMD  &
