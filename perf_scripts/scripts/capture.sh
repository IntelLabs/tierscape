...
if [ ! -z /data/sandeep/git_repo/pcm/build/bin/pcm-memory ]; then
    echo "$0: pcm-memory is available"
    ${PCM_HOME_DIR}/build/bin/pcm-memory ${TREND_SLEEP_DURATION} -silent -nc|grep  "Throughput"|sed 's/--|//g; s/|--//g' >> ${MAIN_LOG_DIR}/pcm_memory &
else
    echo "$0: pcm-memory is not available"
fi
